...
function loadStaffRentals(params = {}) {
    ...
    axios.get(url)
        .then(response => {
            ...
            // Custom Sort Order: repair workflow
            const statusOrder = {
                'pending': 1,
                'approved': 2,
                'in_service': 3,
            ...
            response.data.forEach(rental => {
                ...
                const statusLabel = rental.rental_status.replace(/_/g, ' ');
                ...
                if (rental.rental_status === 'pending') {
                    ...
                    `;
                } else if (rental.rental_status === 'approved') {
                    actions = `
                        <button class="btn btn-sm btn-primary" onclick="updateRentalStatus(${rental.rental_id}, 'in_service')">Move to In Service</button>
                    `;
                } else if (rental.rental_status === 'in_service') {
                    actions = `
                        <button class="btn btn-sm btn-warning" onclick="updateRentalStatus(${rental.rental_id}, 'ready_for_pickup')">Ready for Pickup</button>
                    `;
                } else if (rental.rental_status === 'ready_for_pickup') {
                    actions = `
                        <button class="btn btn-sm btn-success" onclick="updateRentalStatus(${rental.rental_id}, 'picked_up')">Mark Picked Up</button>
                    `;
                } else if (rental.rental_status === 'picked_up') {
                    actions = `
                        <button class="btn btn-sm btn-success" onclick="updateRentalStatus(${rental.rental_id}, 'completed')">Complete Ticket</button>
                    `;
                }
                ...
                const actionButton = actions ? `
                    ...
                    <ul class="dropdown-menu">
                        ...
                        ${rental.rental_status === 'approved' ? `
                            <li><a class="dropdown-item" href="#" onclick="updateRentalStatus(${rental.rental_id}, 'in_service')">Move to In Service</a></li>
                        ` : ''}
                        ${rental.rental_status === 'in_service' ? `
                            <li><a class="dropdown-item" href="#" onclick="updateRentalStatus(${rental.rental_id}, 'ready_for_pickup')">Ready for Pickup</a></li>
                        ` : ''}
                        ${rental.rental_status === 'ready_for_pickup' ? `
                            <li><a class="dropdown-item" href="#" onclick="updateRentalStatus(${rental.rental_id}, 'picked_up')">Mark Picked Up</a></li>
                        ` : ''}
                        ${rental.rental_status === 'picked_up' ? `
                            <li><a class="dropdown-item" href="#" onclick="updateRentalStatus(${rental.rental_id}, 'completed')">Complete Ticket</a></li>
                        ` : ''}
                    </ul>
                ...
                cardsContainer.append(`
                    <div class="card mb-3 shadow-sm">
                        <div class="card-body">
                            ...
                            <div class="mb-2">
                                <strong>Drop-off:</strong> ${new Date(rental.pickup_date).toLocaleDateString()}
                            </div>
                            <div class="mb-2">
                                <strong>Target Pickup:</strong> ${new Date(rental.return_date).toLocaleDateString()}
                            </div>
                            <div class="mb-2">
                                <strong>Estimate:</strong> $${rental.total_price}
                            </div>
                            <div class="mb-3">
...
function getStatusColor(status) {
    switch(status) {
        ...
        case 'ready_for_pickup': return 'warning text-dark';
        case 'picked_up': return 'info text-dark';
